...
async def create_redis_stream(
    ...
    **kwargs
    ...
    return RedisStreamClient(redis_client.client)
...
class RedisStreamClient:
    ...
    async def rebalance_workload(
        ...
        try:
            ...
            # 4. Convert to StreamMessage format
            return [
                StreamMessage(
                    id=msg_id,
                    data=json.loads(msg_data[b'data']),
    ...
    async def acknowledge(
        ...
        *message_ids: str
        ...
        try:
            return await self.redis.client.xack(stream, group, *message_ids)
        except Exception as e:
            logger.error(
                "acknowledge_failed",
                stream=stream,
    ...
    async def trim(self, stream, minid=None, maxlen=None, approximate=True):
